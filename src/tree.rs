...
impl<K: Ord + Clone + Serialize + DeserializeOwned, V: Clone + Serialize + DeserializeOwned> TreeMap<K, V> {
    ...
    fn size(&self) -> usize {
        self.root.as_ref().map_or(0,|node| Self::size_node(&*node))
    }
    fn size_node(node: &Node<K,V>) -> usize {
        1+
         node.left.as_ref().map_or(0,|child| Self::size_node(&*child))+
         node.right.as_ref().map_or(0,|child| Self::size_node(&*child))
    }

    fn depth(&self) -> usize {
        self.root.as_ref().map_or(0,|node| Self::depth_node(&*node))
    }
    fn depth_node(node: &Node<K,V>) -> usize {
        1+std::cmp::max(node.left.as_ref().map_or(0,|child| Self::depth_node(&*child)),
                        node.right.as_ref().map_or(0,|child| Self::depth_node(&*child)))
    }
    ...
    #[inline(never)]
    pub fn get<'a, B: std::cmp::Ord+?Sized>(&'a self, key: &B) -> Option<&'a V> where K: Borrow<B> { Self::get_node(&self.root, key) }
    fn get_node<'a, B: std::cmp::Ord+?Sized>(node: &'a Option<Box<Node<K, V>>>, key: &B) -> Option<&'a V> where K: Borrow<B> {
        match node {
