...
impl<K: Ord + Clone + Serialize + DeserializeOwned, V: Clone + Serialize + DeserializeOwned>
    TreeMap<K, V>
{
    pub fn new() -> Self {
        TreeMap { root: None }
    ...
    #[inline(never)]
    pub fn get<'a, B: std::cmp::Ord + ?Sized>(&'a self, key: &B) -> Option<&'a V>
    where
        K: Borrow<B>,
    {
        Self::get_node(&self.root, key)
    }
    fn get_node<'a, B: std::cmp::Ord + ?Sized>(
    ...
    pub fn seek_ge(&self, key: &K) -> Option<(K, V)> {
        ...
        while let Some(n) = cur {
            if &n.key < key {
                ...
                res = Some((n.key.clone(), n.value.clone()));
                cur = &n.left;
            }
        }
        res
