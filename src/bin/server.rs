...
fn handle_client(args: Arc<Args>, stream: TcpStream, logwriter: Arc<Mutex<File>>, map: Arc<RwLock<TreeMap<KeyType, ValueType>>>)
{
    ...
    while let Some(Ok(line)) = lines.next() {
        ...
        match parts[0] {
            "GET" if parts.len() == 2 => {
                ...
                response.push_str(&match map.get(&parts[1].into()) {
                    Some(v) => format!("OK {}\r\n", v),
                    None    => "ERR NotFound\r\n".into(),
                });
            }
            ...
            "REMOVE" if parts.len() == 2 => {
                let mut map = map.write().unwrap();
                response.push_str(match map.remove(&parts[1].into()) {
                    Some(_) => {
                        if !args.memonly {
                            log.push_str(line.as_str());
                            log.push_str("\n");
            ...
            "SEEK" if parts.len() == 2 => {
                let map = map.read().unwrap();
                response.push_str(&match map.seek_ge(&parts[1].into()) {
                    Some((k, v)) => format!("OK {} {}\r\n", k, v),
                    None          => "ERR NotFound\r\n".into(),
                });
            }
...
fn main() -> std::io::Result<()> {
    ...
    println!("Server listening on {}, using keys of type {}",args.addr,std::any::type_name::<KeyType>());
